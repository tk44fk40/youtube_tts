...
- [1. 概要と使い方](#1-概要と使い方)
  ...
  - [セットアップ手順](#セットアップ手順)
  - [実行方法](#実行方法)
  - [配信モードと過去コメント取得（バックログ）の仕様の違い](#配信モードと過去コメント取得バックログの仕様の違い)
  - [その他のスクリプトとコマンドラインオプション](#その他のスクリプトとコマンドラインオプション)
  - [実行時のコンソール出力](#実行時のコンソール出力)
  - [環境変数設定](#環境変数設定)
  - [オーディオデバイスの確認と指定方法](#オーディオデバイスの確認と指定方法)
  - [設定ファイル](#設定ファイル)
  - [認証・トークンファイル (client_secret.json / token.json)](#認証トークンファイル-client_secretjson--tokenjson)
- [2. ライブラリの概要](#2-ライブラリの概要)
  - [モジュール構成](#モジュール構成)
  - [主要なクラス・関数の説明](#主要なクラス関数の説明)
- [3. テストに関する記述](#3-テストに関する記述)
  - [テストの実行方法](#テストの実行方法)
- [4. ライセンス](#4-ライセンス)
...
2. **VOICEVOX**
   - 本ツールの起動時に Podman または Docker を使用して `voicevox-engine` コンテナが自動で作成・起動・管理されます（複数ツールの並列実行時も安全に管理され、最後のツールが終了した時点で自動停止します）。
   - **事前手動起動は不要です。**
   - ※別途デスクトップアプリの VOICEVOX 等を手動起動して連携させたい場合は、`--no-manage-container` オプションまたは環境変数 `VOICEVOX_MANAGE_CONTAINER=false` を指定してください。
   - ※Podman/Docker が利用できない環境では自動管理が OFF となり、手動で VOICEVOX アプリ/サーバーを起動しておく必要があります（デフォルトポート `50021`）。
...
| `--quota-interval` | (なし) | クォータ使用量を取得・表示する最短時間（秒）。 | `600.0` |
