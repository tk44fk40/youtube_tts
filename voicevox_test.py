...
def main():
    ...
    parser.add_argument(
        "-s", "--speaker", type=int, default=DEFAULT_SPEAKER,
        help="話者スタイルID"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-r", "--samplerate", type=int, default=None,
        help="生成サンプリングレート（未指定時はデバイスの既定値を使用）"
    )
    parser.add_argument(
        "-v", "--volume", type=float, default=DEFAULT_VOLUME,
        help="音量比（デフォルト: 1.0）"
    )
    parser.add_argument(
        "--speed", type=float, default=env_speed,
        help=(
            "読み上げスピード（デフォルト: 1.0）。"
