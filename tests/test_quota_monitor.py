...
def test_format_reset_time_delta_days():
    """format_reset_time で明日やそれ以降の日付が正しくフォーマットされるか検証します。"""
    ...
    with patch("youtube_tts.workers.quota_monitor.datetime") as mock_datetime:
        ...
        assert monitor.format_reset_time(reset_time_future) == "7月3日の17時15分"
...
def test_is_quota_exceeded_error_content_str_or_bytes():
    ...
    with patch("googleapiclient.errors.HttpError.__str__", return_value="quotaExceeded"):
...
def test_check_and_talk_queue_full(mock_get_quota):
    ...
    monitor = QuotaMonitor(app=app, creds="creds", project_id="proj", quota_talk=True, interval=0.0)
