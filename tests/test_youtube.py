...
def test_extract_video_id(mock_creds):
    ...
    assert (
        client.extract_video_id("https://www.youtube.com/live/dQw4w9WgXcQ")
        == "dQw4w9WgXcQ"
    )
    # Case where exception occurs
    #
    # 例外が発生するケース
...
def test_check_stream_active_all_cases(mock_build, mock_creds, caplog):
    ...
    with caplog.at_level("WARNING"):
        assert client.check_stream_active("vid") is True
    assert any(
        "Error checking video status" in record.message
...
def test_handle_comments_disabled_error(mock_build, mock_creds):
    ...
    resp = Response({"status": 403, "reason": "Forbidden"})
    content = (
        b'{"error": {"errors": [{"domain": "youtube.commentThread", '
        b'"reason": "commentsDisabled"}], "code": 403, '
        b'"message": "Comments are disabled"}}'
    )
...
def test_get_my_channel_id_success(mock_build, mock_creds):
    ...
    # mock_service.channels().list is called once, but the mock
    # might record multiple sub-calls. Since we patch at the build
    # level, mock_service.channels().list() will be called twice
    # for the return value, but the .list() call itself is cached.
    # Verify that channels().list was called once.
    # The actual call is client.youtube.channels().list(...)
    # Let's count mock calls. We reset the mock list to be sure.
    #
    # キャッシュにより1回だけ呼ばれたことを確認
    # mock_service.channels().list は1回だけ呼び出されますが、
    # モックは複数のサブコールを記録する可能性があります。
    # ビルドレベルでパッチを適用しているため、戻り値取得のために
    # mock_service.channels().list() が2回呼び出されますが、
    # .list() の呼び出し自体はキャッシュされます。
    # channels().list が1回だけ呼び出されたことを検証します。
    # 実際の呼び出しは client.youtube.channels().list(...) です
    # モックの呼び出し数をカウントします。
    # 確実にするためにモックのリストをリセットします。
...
def test_get_my_channel_id_failure(mock_build, mock_creds, caplog):
    ...
    assert any(
        "Failed to get my channel ID" in record.message
        for record in caplog.records
    )
...
def test_fetch_comment_threads_success(mock_build, mock_creds):
    ...
    client = YouTubeChatClient(mock_creds)
    items, next_token, interval = client.fetch_comment_threads(
        "vid123", max_results=50
    )
    assert len(items) == 1
...
def test_fetch_comment_threads_parse_error(mock_build, mock_creds, caplog):
    ...
    mock_service.commentThreads().list().execute.return_value = {
        "items": [
            ...
            # Pass non-dict value to trigger AttributeError
            #
    ...
    assert any(
        "Failed to parse comment" in record.message for record in caplog.records
    )
