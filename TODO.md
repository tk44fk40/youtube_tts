...
- わかりやすさを考慮したはずが、逆に助長になってしまってる可能性がある
- 本来あるべきテスト趣旨にそぐわない、おかしなテスト内容になってるかも
  - 例：クォータ判定中に想定外の例外が起きた場合のテストで、本来ありえない例外オ
    ブジェクト内の status プロパティが無い例外オブジェクトを返すことで、強引に
    例外を出していた
    - 例外自体は起こるものの、普通はその想定外の例外はちゃんと例外オブジェクトと
      して成立していて status も持っている
    - 副作用として、たまたまテスト対象箇所が、例外ブロックの中で status を取得
      するようになっていたので、例外の中で更に例外が起こるという分かりづらい状況
      になった
- テストコード肥大化、要分割
